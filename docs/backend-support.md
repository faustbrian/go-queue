...
| Terminal dead letter | N/A | No | Append-before-ack stream | Append-before-ack stream | No | Package-owned terminal publish before FIN | Confirmed configured terminal publish before source ack |
...
| Same-worker reconnect | N/A | Yes, without replay | Client reconnect; backlog retained | Subsequent commands reconnect | Yes, without replay | Yes | Bounded native recovery |
| Proven topology | Process | Standalone, Sentinel, cluster | Standalone, cluster | Standalone Valkey 9 only | Core server | nsqd | Operator-owned passive topology; RabbitMQ 4.3.5 TLS CI |
