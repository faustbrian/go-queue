...
`go-queue` provides Horizon-like failed-job operations without reproducing
...
| Horizon concept | `go-queue` contract | Intentional difference |
| --- | --- | --- |
...
5. Recreate Horizon roles in `go-queue-control-plane`. Authorization, audit,
...
7. Freeze Horizon retries, drain its active queues, retain its failed-job store
   for the required audit window, then enable bounded `go-queue` redrive.
...
attempt and completed terminal settlement. `go-queue` exposes both records and
