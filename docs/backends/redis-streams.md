...
is visible. Malformed and oversized entries remain pending for operator action.
Integration evidence uses Redis 6.2.22 and `go-redis/v9` 9.19.0.
Queued backlog is retained across a same-endpoint Redis restart; automatic PEL
claiming remains outside this worker's policy.
