...
| M-01 | Medium | RabbitMQ | Malformed manual-ack delivery remained unsettled | Infinite poison requeue | Fixed with terminal nack without requeue |
...
statement coverage remains 100%, decoder and message-state fuzz targets cover
every network wrapper, and race/static/vulnerability/workflow gates are part of
the final command set in this report.

## Release verdict

The 2026-07-14 audit verdict is **go** for a pre-v1 hardening release. Every
...
| `FUZZ_TIME=1s scripts/check-fuzz.sh` | Pass for all seven targets |
| `BENCH_TIME=10x make benchmark` | Pass |
| `go test -tags=integration -count=1 -timeout=15m ./...` | Pass without skips, including Sentinel and broker restart scenarios |
