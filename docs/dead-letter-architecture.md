...
`go-queue-control-plane` authorization, audit, and presentation concerns.

## Ownership boundary

`go-queue` owns failure classification, delivery settlement, durable
...
- `go-queue-control-plane` consumes these contracts without acquiring Redis or
