...
The `management` package is the stable boundary between `go-queue` workers and
...
`Command` is the stable envelope that a management-capable `go-queue` adapter
...
backend-specific semantics remain owned by `go-queue` adapters.
