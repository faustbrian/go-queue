...
bounded only by broker and operational policy. Configure broker-side quotas,
retention, dead-lettering, and connection limits for the deployment.
...
execution. Redis Streams leaves malformed work pending for explicit operator
recovery; NSQ finishes it; RabbitMQ rejects it without requeue when manual
