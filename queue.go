...
func (q *Queue) work(task core.TaskMessage) {
	...
	defer func() {
		...
		if e != nil {
			...
			q.safeLogErrorf("handler panic: %v", e)
		}
		q.schedule()
	...
	if err = q.run(task); err != nil {
		q.safeLogErrorf("runtime error: %s", err.Error())
	}
...
func (q *Queue) settle(task core.TaskMessage, handlerErr error) error {
	...
	if err := invokeSettlement("reject delivery", delivery.Nack); err != nil {
...
func invokeSettlement(operation string, settle func() error) (err error) {
	defer func() {
		if panicValue := recover(); panicValue != nil {
			err = fmt.Errorf("%s panic: %v", operation, panicValue)
		}
	}()
	return settle()
}
