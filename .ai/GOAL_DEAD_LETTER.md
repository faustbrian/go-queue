...
This is a follow-up execution goal for the in-progress `go-queue`
...
`go-queue`, not an optional future helper.
...
- stable record, inspection, retry, replay, delete, purge, and capability
  contracts consumed by `go-queue-control-plane`;
- honest backend-specific guarantees, limitations, and recovery semantics.

The package MUST remain fully usable without `go-queue-control-plane`.
...
package observation hooks and `go-telemetry` without a mandatory exporter.
...
- Administrative authorization and auditing belong to
  `go-queue-control-plane`; data-plane contracts MUST carry enough bounded
...
- end-to-end contract tests with `go-queue-control-plane`.
...
5. Integrate the concrete contracts with `go-queue-control-plane`.
