...
- stable worker-status, management, control-command, failed-job, dead-letter,
  and capability contracts for `go-queue-control-plane`
...
- `go-idempotency` middleware integration
...
- administrative API, CLI, web UI, historical dashboard, fleet inventory, or
  Kubernetes scaling integration; these belong to `go-queue-control-plane`
...
`go-queue` owns the data-plane side of control-plane interoperability:
...
`go-queue` MUST remain fully operational without a running control plane. It
...
- `go-queue-control-plane` protocol and integration guide
- `go-idempotency` consumer integration guide
