...
func TestSettlementJoinsHandlerAndRejectionFailures(t *testing.T) {
	...
	message.SetAcknowledgement(nil, func() error { return errors.New("nack failed") })

	err = q.settle(&message, errors.New("handler failed"))

	assert.ErrorContains(t, err, "handler failed")
	assert.ErrorContains(t, err, "nack failed")
