...
func (w *Worker) listRecords(
	...
	items := make([]management.JobRecord, 0, management.MaxPageSize)
	...
	for _, message := range messages {
		...
		if matchesRedisRecord(record, request.Search) && itemCount < request.Limit {
			items = append(items, record)
			itemCount++
		}
	}
	page := management.RecordPage{Items: items}
	if int64(len(messages)) == scanLimit {
