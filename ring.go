...
type Ring struct {
	...
	exitOnce  sync.Once                                     // Makes the drained signal durable and idempotent
...
func NewRing(opts ...Option) *Ring {
	o := NewOptions(opts...)
	if o.queueSize < 0 {
		panic(fmt.Errorf("%w: queue size cannot be negative", ErrInvalidConfiguration))
	}
	w := &Ring{
