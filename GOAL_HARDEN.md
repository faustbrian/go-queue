# Goal: Audit and harden `go-queue`

## Mission

Perform an evidence-driven correctness, resilience, security, and operational
audit of `go-queue` and every bundled backend, then implement the hardening
...
- The repository's `GOAL.md`, `AGENTS.md`, delivery-semantics and backend
  matrices, migration guide, public API, tests, benchmarks, and changelog.
- Primary security guidance for Redis, NATS, NSQ, RabbitMQ, credentials, TLS,
  deserialization, and denial-of-service resistance.
...
2. Build a delivery-semantics matrix for in-memory, Redis Pub/Sub, Redis
   Streams, NATS, NSQ, and RabbitMQ using code and integration evidence.
...
prove durable settlement behavior for Redis Streams, NSQ, and RabbitMQ. Never
describe best-effort delivery as durable or exactly-once.
