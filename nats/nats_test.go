...
func TestCoreNATSReconnectsAfterBrokerRestart(t *testing.T) {
	ctx := context.Background()
	natsC, endpoint := setupNatsContainer(ctx, t)
	defer testcontainers.CleanupContainer(t, natsC)

	worker := NewWorker(WithAddr(endpoint), WithSubj("restart"), WithQueue("restart"))
	...
	require.NoError(t, natsC.Stop(ctx, &stopTimeout))
	require.Eventually(t, func() bool {
		status := worker.client.Status()
		return status == natsgo.RECONNECTING || status == natsgo.DISCONNECTED
	}, 5*time.Second, time.Millisecond)
	require.NoError(t, natsC.Start(ctx))
	require.Eventually(t, func() bool {
