...
func TestNativeTransportReplayPoliciesAndBoundedRegistry(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, nativeReplayOK, outcome)
	registrySize, err := client.Do(ctx, client.B().Hlen().Key("archive:go-queue:replay-index").Build()).ToInt64()
	require.NoError(t, err)
	assert.Equal(t, int64(1), registrySize)
	orderSize, err := client.Do(ctx, client.B().Zcard().Key("archive:go-queue:replay-order").Build()).ToInt64()
	require.NoError(t, err)
